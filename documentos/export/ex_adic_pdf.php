...
$objdetalle->imprimir_det_adicionales();
$resdet_1 = $adi_det;
$pdf = new PDF();
$pdf->AliasNbPages();
...
$pdf->Cell(30, 5, $cant.' '.'ADICIONALES GENERADOS', 0, 0, 'R');
$pdf->ln(8);        
$n=1;
while ($rowdet_1 = mysqli_fetch_array($resdet_1)) {
    ...
    include_once '../../class/trandetalle.php';
    $objdetalle = new Trandetalle();
    $objdetalle->ver_detalle_adi($id_ref);
    $resdet_2 = $dtl;
    $mensaje='ADICIONAL # '.$n.' POR EL VALOR DE '. $rowdet_1['tran_det_monto'].' A '.$rowdet_1['tran_det_plazo'].' DIAS PLAZO CON INTERES DE '.$rowdet_1['tran_det_interes'].'%';
    $pdf->Cell(120, 5, $mensaje.' '.'', 0, 0, 'R');
    $pdf->ln(5);        
    $pdf->Cell(20, 5, '#', 0, 0, 'R');
...
$pdf->SetFont('Arial', '', 8);//10
$pdf->Cell(20, 5, $rowdet_2[0].'/'.$cant, 0, 0, 'R');
